...
class Dialog(Scene):
  ...
  def __init__(self, viewer_dialog:bool, text:str, delay_ns:int):
    Scene.__init__(self)
    self.stage:Stage = None # TODO: This could probably be pushed up to Scene..
    self.viewer_dialog:bool = viewer_dialog
    self.cooldown_scene = delay_ns
    self.text:str = text
  ...
  # TODO: And this could be convered to an on_open(self), where the stage is available as a property already.
  ...
  def on_update(self, delta_nanos:int):
    ending = self.characters_remaining <= 0 and self.cooldown_scene <= 0
    ...
    if not ending:
      ...
      if typing:
        ...
        self.cooldown_scene -= delta_nanos
